...
public class TekeningTest {
    ...
    @Test (expected = DomainException.class)
    public void test_VoegToe_MagNietGelijkZijnAanNull_GooiDomainException(){
        Tekening huis = new Tekening("huis");
        huis.voegToe(null);
    }

    @Test (expected = DomainException.class)
    public void test_VoegToe_Dezelfde_GooiDomainException(){
        Tekening huis = new Tekening("huis");
        huis.voegToe(gebouw);
        huis.voegToe(gebouw);
    }
    ...
    @Test (expected = DomainException.class)
    public void test_GetVorm_Met_intKleinerDan0_GooiDomainException(){
        Tekening huis = createHuisMetSchouw();
        huis.getVorm(-5);
    }

    @Test (expected = DomainException.class)
    public void test_GetVorm_Met_intGroterDanAantalVormen_GooiDomainException(){
        Tekening huis = createHuisMetSchouw();
        huis.getVorm(100);
    }

    @Test
    public void test_GetVorm_Met_CorrecteWaarde(){
        Tekening huis = createHuisMetSchouw();
        assertEquals(raambalk1, huis.getVorm(5));
    }

    @Test (expected = DomainException.class)
    public void test_Verwijder_ElementNiet_In_Lijst_GooiDomainException(){
        Tekening huis = createHuisMetSchouwZonderDeur();
        huis.verwijder(deur);
    }

    @Test
    public void test_Verwijder_Met_Geldig_Element(){
        Tekening huis = createHuisMetSchouwZonderDeur();
        huis.verwijder(dak);
    }
